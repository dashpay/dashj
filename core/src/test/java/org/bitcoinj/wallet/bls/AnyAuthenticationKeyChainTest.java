...
public class AnyAuthenticationKeyChainTest {
    Context context;
    ...
    public void authenticationKeyChainTest() {
        ...
        AuthenticationKeyChain owner = new AuthenticationKeyChain(seed, AnyDeterministicKeyChain.PROVIDER_OWNER_PATH_TESTNET);
        DeterministicKey myOwnerKey = owner.getKey(KeyChain.KeyPurpose.AUTHENTICATION);
        assertEquals(myOwnerKey, ownerKey);

        AuthenticationKeyChain voting = new AuthenticationKeyChain(seed, AnyDeterministicKeyChain.PROVIDER_VOTING_PATH_TESTNET);
        DeterministicKey myVotingKey = voting.getKey(KeyChain.KeyPurpose.AUTHENTICATION);
        assertEquals(myVotingKey, votingKey);

        AuthenticationKeyChain blockchainUser = new AuthenticationKeyChain(seed, AnyDeterministicKeyChain.BLOCKCHAIN_USER_PATH_TESTNET, AuthenticationKeyChain.KeyChainType.BLOCKCHAIN_IDENTITY, true);
        DeterministicKey myBlockchainUserKey = blockchainUser.getKey(0, true);
        assertEquals(myBlockchainUserKey, buKey);
        DeterministicKey bukeyIndexZero = blockchainUser.getKey(0, true);
