...
public class WalletTest extends TestWithWallet {
    ...
    public void basicSpendingToP2SH() throws Exception {
        Address destination = Address.fromScriptHash(UNITTEST, HEX.decode("4a22c3c4cbb31e4d03b15550636762bda0baf85a"));
    ...
    // identical result to Dash Core. However the signatures are not deterministic - signing the same data
    ...
    public void blockChainCatchup() throws Exception {
        ...
        // What we'd really like to do is prove Dash Core would accept it .... no such luck unfortunately.
    ...
    public void feeSolverAndCoinSelectionTests2() throws Exception {
        ...
        assertEquals(Coin.valueOf(1213), request15.tx.getFee());
        ...
        assertEquals(Coin.valueOf(999), request17.tx.getFee());
        ...
        assertEquals(Coin.valueOf(1001), request18.tx.getFee());
    ...
    public void recipientPaysFees() throws Exception {
        ...
        request.feePerKb = Transaction.DEFAULT_TX_FEE.multiply(10);
        request.ensureMinRequiredFee = true;
        ...
        request2.feePerKb = Transaction.DEFAULT_TX_FEE.multiply(10);
        ...
        // Hardcoded tx length because actual length may vary depending on actual signature length
        Coin fee5 = Transaction.DEFAULT_TX_FEE.multiply(261).divide(1000).multiply(10);
        ...
        request5.feePerKb = Transaction.DEFAULT_TX_FEE.multiply(10);
    ...
    public void transactionGetFeeTest() throws Exception {
        ...
        wallet.completeTx(request);
        assertEquals(Coin.valueOf(227), request.tx.getFee());
    }
    ...
    public void lowerThanDefaultFee() throws InsufficientMoneyException {
        ...
        req.feePerKb = fee;
        wallet.completeTx(req);
        assertEquals(Coin.valueOf(227).divide(feeFactor), req.tx.getFee());
        wallet.commitTx(req.tx);
    ...
    public void higherThanDefaultFee() throws InsufficientMoneyException {
        ...
        req.feePerKb = fee;
        wallet.completeTx(req);
        assertEquals(Coin.valueOf(227).multiply(feeFactor), req.tx.getFee());
        wallet.commitTx(req.tx);
        SendRequest emptyReq = SendRequest.emptyWallet(myAddress);
        emptyReq.feePerKb = fee;
        emptyReq.emptyWallet = true;
        emptyReq.allowUnconfirmed();
        wallet.completeTx(emptyReq);
        assertEquals(Coin.valueOf(3420), emptyReq.tx.getFee());
        wallet.commitTx(emptyReq.tx);
    }
    ...
    public void testCompleteTxWithExistingInputs() throws Exception {
        ...
        request2.tx.getInput(0).getScriptSig().correctlySpends(
                request2.tx, 0, null, tx3.getOutput(0).getScriptPubKey(), Script.ALL_VERIFY_FLAGS);
        ...
        for (int i = 0; i < req.tx.getInputs().size(); i++) {
            ...
            if (input.getConnectedOutput().getParentTransaction().equals(t1)) {
                ...
                input.getScriptSig().correctlySpends(
                        req.tx, i, null, t3.getOutput(0).getScriptPubKey(), Script.ALL_VERIFY_FLAGS);
            }
        }
