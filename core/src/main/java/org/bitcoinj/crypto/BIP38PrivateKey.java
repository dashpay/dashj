...
public class BIP38PrivateKey extends PrefixedChecksummedBytes {
    ...
    public ECKey decrypt(String passphrase) throws BadPassphraseException {
        ...
        Sha256Hash hash = Sha256Hash.twiceOf(Address.fromKey(params, key).toString().getBytes(StandardCharsets.US_ASCII));
    ...
    private ECKey decryptNoEC(String normalizedPassphrase) {
        try {
            byte[] derived = SCryptWrapper.scrypt(normalizedPassphrase.getBytes(StandardCharsets.UTF_8), addressHash, 16384, 8, 8, 64);
    ...
    private ECKey decryptEC(String normalizedPassphrase) {
        try {
            ...
            byte[] passFactorBytes = SCryptWrapper.scrypt(normalizedPassphrase.getBytes(StandardCharsets.UTF_8), ownerSalt, 16384, 8, 8, 32);
            if (hasLotAndSequence) {
            ...
            byte[] derived = SCryptWrapper.scrypt(k.getPubKey(), salt, 1024, 1, 1, 64);
