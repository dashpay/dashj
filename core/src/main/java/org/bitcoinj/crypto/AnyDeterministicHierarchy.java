 ...
 * ability to hand out {@link BLSDeterministicKey}s to other people who can then create new addresses
...
public class AnyDeterministicHierarchy {
     ...
     */
    public AnyDeterministicHierarchy(IDeterministicKey rootKey) {
        putKey(rootKey);
    ...
    public IDeterministicKey get(List<ChildNumber> path, boolean relativePath, boolean create) {
        ...
        if (!keys.containsKey(absolutePath)) {
            ...
            IDeterministicKey parent = get(absolutePath.subList(0, absolutePath.size() - 1), false, true);
            putKey(parent.deriveChildKey(absolutePath.get(absolutePath.size() - 1)));
        }
     ...
     * @throws IllegalArgumentException if the parent doesn't exist and createParent is false.
     */
    public IDeterministicKey deriveChild(List<ChildNumber> parentPath, boolean relative, boolean createParent, ChildNumber createChildNumber) {
        return deriveChild(get(parentPath, relative, createParent), createChildNumber);
    }

    private IDeterministicKey deriveChild(IDeterministicKey parent, ChildNumber createChildNumber) {
        IDeterministicKey childKey = parent.deriveChildKey(createChildNumber);
        putKey(childKey);
        return childKey;
    }

    /**
     * Returns the root key that the {@link AnyDeterministicHierarchy} was created with.
     */
    public IDeterministicKey getRootKey() {
