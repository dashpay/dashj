...
import org.bitcoinj.wallet.bls.AnyBasicKeyChain;
import org.bitcoinj.wallet.bls.AnyDefaultKeyChainFactory;
...
public class AnyAuthenticationKeyChainGroup extends AnyKeyChainGroup {

    private static final Logger log = LoggerFactory.getLogger(AnyAuthenticationKeyChainGroup.class);
    ...
    public static class Builder {
         ...
         */
        public AnyAuthenticationKeyChainGroup.Builder fromRandom(Script.ScriptType outputScriptType) {
         ...
         * @param outputScriptType type of addresses (aka output scripts) to generate for receiving
         */
        public AnyAuthenticationKeyChainGroup.Builder fromSeed(DeterministicSeed seed, Script.ScriptType outputScriptType) {
         ...
         * @param chain to add
         */
        public AnyAuthenticationKeyChainGroup.Builder addChain(AnyDeterministicKeyChain chain) {
            this.chains.add(chain);
            return this;
        }
         ...
         * @param chains to add
         */
        public AnyAuthenticationKeyChainGroup.Builder chains(List<AnyDeterministicKeyChain> chains) {
            this.chains.clear();
         ...
         */
        public AnyAuthenticationKeyChainGroup.Builder lookaheadSize(int lookaheadSize) {
         ...
         */
        public AnyAuthenticationKeyChainGroup.Builder lookaheadThreshold(int lookaheadThreshold) {
            this.lookaheadThreshold = lookaheadThreshold;
            return this;
        }

        public AnyAuthenticationKeyChainGroup build() {
            return new AnyAuthenticationKeyChainGroup(params, null, chains, lookaheadSize, lookaheadThreshold, null, null, keyFactory);
        }
    }

    protected AnyAuthenticationKeyChainGroup(NetworkParameters params, @Nullable AnyBasicKeyChain basicKeyChain, List<AnyDeterministicKeyChain> chains, int lookAheadSize, int lookAheadThreshold,
    ...
    public static AnyAuthenticationKeyChainGroup.Builder authenticationBuilder(NetworkParameters params) {
        return new Builder(params, KeyChainGroupStructure.DEFAULT);
    }

    public AnyAuthenticationKeyChain getKeyChain(AuthenticationKeyChain.KeyChainType type) {
        for(AnyDeterministicKeyChain chain : chains) {
            if(((AnyAuthenticationKeyChain)chain).getType() == type)
                return (AnyAuthenticationKeyChain)chain;
        }
        return null;
    }
    ...
    public void addAndActivateHDChain(AnyDeterministicKeyChain chain) {
        if(chain instanceof AnyAuthenticationKeyChain)
    ...
    static AnyAuthenticationKeyChainGroup fromProtobufUnencrypted(NetworkParameters params, List<Protos.Key> keys, AuthenticationKeyChain.KeyChainType type) throws UnreadableWalletException {
        return fromProtobufUnencrypted(params, keys, new AnyDefaultKeyChainFactory(), type);
    }

    public static AnyAuthenticationKeyChainGroup fromProtobufUnencrypted(NetworkParameters params, List<Protos.Key> keys, AnyKeyChainFactory factory, AuthenticationKeyChain.KeyChainType type) throws UnreadableWalletException {
        KeyFactory keyFactory = AnyAuthenticationKeyChain.getKeyFactory(type);
        ...
        for(AnyDeterministicKeyChain chain : chains) {
            Preconditions.checkState(chain instanceof AnyAuthenticationKeyChain);
        }
        ...
        return new AnyAuthenticationKeyChainGroup(params, basicKeyChain, chains, lookaheadSize, lookaheadThreshold, currentKeys, null, keyFactory);
    }

    static AnyAuthenticationKeyChainGroup fromProtobufEncrypted(NetworkParameters params, List<Protos.Key> keys, KeyCrypter crypter, AuthenticationKeyChain.KeyChainType type) throws UnreadableWalletException {
        return fromProtobufEncrypted(params, keys, crypter, new AnyDefaultKeyChainFactory(), type);
    }

    public static AnyAuthenticationKeyChainGroup fromProtobufEncrypted(NetworkParameters params, List<Protos.Key> keys, KeyCrypter crypter, AnyKeyChainFactory factory, AuthenticationKeyChain.KeyChainType type) throws UnreadableWalletException {
        ...
        return new AnyAuthenticationKeyChainGroup(params, basicKeyChain, chains, lookaheadSize, lookaheadThreshold, currentKeys, crypter, keyFactory);
    }
    ...
    public IDeterministicKey currentKey(AuthenticationKeyChain.KeyChainType type) {
        ...
        AuthenticationKeyChain.KeyChainType accountPath = ((AnyAuthenticationKeyChain) chain).getType();
    ...
    protected static HashMap<AuthenticationKeyChain.KeyChainType, IDeterministicKey> createCurrentAuthenticationKeysMap(List<AnyDeterministicKeyChain> chains) {
        ...
        for(AnyDeterministicKeyChain chain : chains) {
            AnyAuthenticationKeyChain contactChain = (AnyAuthenticationKeyChain)chain;
            ...
            if (contactChain.getIssuedExternalKeys() > 0) {
                ...
                currentKeys.put(((AnyAuthenticationKeyChain)chain).getType(), currentExternalKey);
            }
        }
        return currentKeys;
    }
    ...
    protected AuthenticationKeyChain.KeyChainType getKeyChainType(byte [] pubkeyHash) {
        for (AnyDeterministicKeyChain chain: chains) {
            if (chain.findKeyFromPubHash(pubkeyHash) != null) {
                return ((AnyAuthenticationKeyChain)chain).getType();
            }
        }
