 ...
 *
 * <p>This class builds on {@link AnyDeterministicHierarchy} and
...
public class AnyDeterministicKeyChain implements IEncryptableKeyChain {
    ...
    /** Returns the deterministic key for the given absolute path in the hierarchy. */
    public IDeterministicKey getKeyByPath(List<ChildNumber> path) {
    ...
    public IDeterministicKey getKeyByPath(List<ChildNumber> path, boolean create) {
    ...
    protected List<Protos.Key> serializeMyselfToProtobuf() {
        ...
        Map<IKey, Protos.Key.Builder> keys = basicKeyChain.serializeToEditableProtobufs();
        for (Map.Entry<IKey, Protos.Key.Builder> entry : keys.entrySet()) {
    ...
    public void maybeLookAhead() {
        lock.lock();
        try {
            List<IDeterministicKey> keys = maybeLookAhead(externalParentKey, issuedExternalKeys);
     ...
     */
    private List<IDeterministicKey> maybeLookAhead(IDeterministicKey parent, int issued, int lookaheadSize, int lookaheadThreshold) {
        checkState(lock.isHeldByCurrentThread());
        ...
        List<IDeterministicKey> result  = new ArrayList<>(needed);
    ...
    /* package */ List<IDeterministicKey> getKeys(boolean includeLookahead, boolean includeParents) {
        List<IKey> keys = basicKeyChain.getKeys();
        List<IDeterministicKey> result = new LinkedList<>();
        if (!includeLookahead) {
     ...
     */
    public List<IDeterministicKey> getIssuedReceiveKeys() {
        final List<IDeterministicKey> keys = new ArrayList<>(getKeys(false, false));
        for (Iterator<IDeterministicKey> i = keys.iterator(); i.hasNext();) {
     ...
     */
    public List<IDeterministicKey> getLeafKeys() {
        ImmutableList.Builder<IDeterministicKey> keys = ImmutableList.builder();
